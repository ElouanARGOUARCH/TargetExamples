...
class ConditionalDensityEstimationTarget:
    def __init__(self, choice, num_samples):
        ...
        if choice == 'Deformed Two circles':
            ...
            self.T_prior = MultivariateNormal(torch.zeros(2), torch.eye(2))
            self.T_samples = self.T_prior.sample([num_samples])
            self.X_samples = torch.tensor(X)[torch.randperm(X.shape[0])].float() * self.T_samples
            self.simulator = lambda thetas: torch.cat([torch.tensor(X[torch.randperm(X.shape[0])][0]).unsqueeze(0).float()* torch.abs(theta) for theta in thetas], dim =0)
